...
def test(victim_model, victim_optimizer, type="Clean"):
    ...
    for i in range(args.epochs):
        ...
        loss_u = F.binary_cross_entropy_with_logits(output[idx_train], labels[idx_train].unsqueeze(1).float().to(device))
        train_filt = np.zeros(features.shape[0], dtype=np.bool_)
        train_filt[idx_train] = True
        sens0 = (sens == 0)
        sens1 = (sens == 1)
        logits = torch.sigmoid(output)
        loss_f = torch.abs(logits[sens0].mean() - logits[sens1].mean())
        loss = loss_u + args.alpha * loss_f
        loss.backward()
        ...
        if i % 100 == 0:
            ...
            print(f"iteration {i}, loss: {loss.item():.4f}, loss_reg: {args.alpha * loss_f.item():.4f}, val_acc: {utils.accuracy1(output[idx_val], labels[idx_val]):.4f}, parity: {parity:.4f}, equality: {equality:.4f}, train_acc: {utils.accuracy1(output[idx_train], labels[idx_train]):.4f}, train_roc_auc: {roc_auc_score(labels.cpu().numpy()[idx_train], output.detach().cpu().numpy()[idx_train]):.4f}")
...
def main():
    ...
    best_result = dict()
    victim_model = GCN(nfeat, args.nhid, nclass, args.dropout)
    torch.save(victim_model.state_dict(), 'pre_processed/poisoning_reg_'+args.dataset+'.pt')
    for type in ['Clean', 'Poisson']:
        victim_model.load_state_dict(torch.load('pre_processed/poisoning_reg_'+args.dataset+'.pt'))
        victim_model = victim_model.to(device)
        victim_optimizer = optim.Adam(victim_model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
