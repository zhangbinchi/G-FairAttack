...
for i in range(args.epochs):
    ...
    victim_model.eval()
    output = victim_model(features, edge_clean)
    loss_u_val = F.binary_cross_entropy_with_logits(output[idx_val], labels[idx_val].unsqueeze(1).float().to(device))
    logits_val = torch.sigmoid(output)
    loss_f_val = torch.abs(logits_val[sens0].mean() - logits_val[sens1].mean())
    loss_val = loss_u_val + args.alpha * loss_f_val
    ...
    if i % 100 == 0:
        ...
        print(f"iteration {i}, loss: {loss.item():.4f}, loss_reg: {args.alpha * loss_f.item():.4f}, test_acc: {utils.accuracy1(output[idx_test], labels[idx_test]):.4f}, test_roc_auc: {roc_auc_score(labels.cpu().numpy()[idx_test], output.detach().cpu().numpy()[idx_test]):.4f}, test_parity: {parity:.4f}, test_equality: {equality:.4f}")
